...
/*============================================================================*/
<<<<<<< HEAD
/* graph.cpp / 0-113_nofutu                                                   */
=======
/* graph.cpp / 0-111_nofutu                                                   */
>>>>>>> 3587ec488605672e51133d66eff0e092dcb9204c
/*----------------------------------------------------------------------------*/
//
...
// s_Vertex
    ...
    s_Vertex::s_Vertex(sInt_32 id, sDouble capacity)
	: m_id(id)
	, m_capacity(capacity)
    {
	// nothing	
    ...
    s_Vertex::s_Vertex(const s_Vertex &vertex)
	: m_id(vertex.m_id)
	, m_capacity(vertex.m_capacity)
    {	
	sASSERT(m_in_Neighbors.empty() && vertex.m_out_Neighbors.empty());
    }


    const s_Vertex& s_Vertex::operator=(const s_Vertex &vertex)
    {
	sASSERT(m_in_Neighbors.empty() && vertex.m_out_Neighbors.empty());
...
/*----------------------------------------------------------------------------*/
    ...
    sInt_32 s_Vertex::calc_InNeighborCount(void) const
    {
	return m_in_Neighbors.size();
    }

    
    sInt_32 s_Vertex::calc_OutNeighborCount(void) const
    {
	return m_in_Neighbors.size();
    }    
...
/*----------------------------------------------------------------------------*/
    ...
    void s_Vertex::to_Screen(const sString &indent) const
    {
	to_Stream(stdout, indent);
    }


    void s_Vertex::to_Stream(FILE *fw, const sString &indent) const
    {
	fprintf(fw, "%sVertex: (id = %d, capacity = %.3f)", indent.c_str(), m_id, m_capacity);
	
	if (!m_in_Neighbors.empty() || !m_out_Neighbors.empty())
	{
	    fprintf(fw, " ->[");
	    for (Neighbors_vector::const_iterator neighbor = m_in_Neighbors.begin(); neighbor != m_in_Neighbors.end(); ++neighbor)
	    {
		fprintf(fw, "%d ", (*neighbor)->m_source->m_id);
	    }
	    fprintf(fw, "]");
	    ...
	    fprintf(fw, " [");
	    for (Neighbors_vector::const_iterator neighbor = m_out_Neighbors.begin(); neighbor != m_out_Neighbors.end(); ++neighbor)
	    {
		fprintf(fw, "%d ", (*neighbor)->m_target->m_id);
	    }
	    fprintf(fw, "]->");
...
// s_Arc
    ...
    s_Arc::s_Arc(sInt_32 id, s_Vertex *source, s_Vertex *target)
	: m_id(id)
	, m_source(source)
	, m_target(target)
    {
    ...
    s_Arc::s_Arc(sInt_32 id, s_Vertex *source, s_Vertex *target, sDouble capacity)
	: m_id(id)
	, m_source(source)
	, m_target(target)
	, m_capacity(capacity)
    ...
    s_Arc::s_Arc(const s_Arc &arc)
	: m_id(arc.m_id)
	, m_source(arc.m_source)
	, m_target(arc.m_target)
	, m_capacity(arc.m_capacity)
    {
	// nothing
    }


    const s_Arc& s_Arc::operator=(const s_Arc &arc)
    {
	m_id = arc.m_id;
    ...
    void s_Arc::to_Screen(const sString &indent) const
    {
	to_Stream(stdout, indent);
    }

	 
    void s_Arc::to_Stream(FILE *fw, const sString &indent) const
    {
	fprintf(fw, "%sArc: (id = %d, source = %d, target = %d, capacity = %.4f)\n", indent.c_str(), m_id, m_source->m_id, m_target->m_id, m_capacity);
    }    
...
// s_DirectedGraph
    ...
    s_DirectedGraph::s_DirectedGraph()
	: m_Arc_cnt(0)
    {
	// nothing
    }
    ...
    s_DirectedGraph::s_DirectedGraph(sInt_32 N_vertices)
	: m_Arc_cnt(0)
    {
       	add_Vertices(N_vertices);
    }
    ...
    const s_DirectedGraph& s_DirectedGraph::operator=(const s_DirectedGraph &directed_graph)
    {
	m_Arc_cnt = 0;
...
/*----------------------------------------------------------------------------*/
    ...
    void s_DirectedGraph::generate_UndirectedWaxman(sInt_32 N_vertices, sDouble alpha, sDouble beta, sInt_32 x_grid_max, sInt_32 y_grid_max)
    {
	WaxmanNodes_vector waxman_Nodes;
	WaxmanNodes_vector selected_Nodes;

	for (sInt_32 x = 0; x < x_grid_max; ++x)
	{
	    for (sInt_32 y = 0; y < y_grid_max; ++y)
	    {
		waxman_Nodes.push_back(WaxmanNode(x, y));
	    }
	}

	for (sInt_32 i = 0; i < N_vertices; ++i)
	{
	    sInt_32 rnd_index = random() % waxman_Nodes.size();

	    selected_Nodes.push_back(waxman_Nodes[rnd_index]);
	    waxman_Nodes[rnd_index] = waxman_Nodes[waxman_Nodes.size() - 1];
	    waxman_Nodes.pop_back();
	}

	sDouble max_distance = 0;
	
	for (sInt_32 u = 1; u < selected_Nodes.size(); ++u)
	{
	    for (sInt_32 v = 0; v < u; ++v)
	    {
		sDouble dx = sABS(selected_Nodes[u].m_x - selected_Nodes[v].m_x);
		sDouble dy = sABS(selected_Nodes[u].m_y - selected_Nodes[v].m_y);
	...
	for (sInt_32 u = 0; u < selected_Nodes.size(); ++u)
	{
	    sDouble rnd2 = random() / (sDouble)RAND_MAX;	    
	...
	for (sInt_32 u = 1; u < selected_Nodes.size(); ++u)
	{
		...
		sDouble distance = sqrt(dx * dx + dy * dy);
		sDouble probability = beta * exp(-distance / (alpha * max_distance));

		sDouble rnd1 = random() / (sDouble)RAND_MAX;
		sDouble rnd2 = random() / (sDouble)RAND_MAX;

		sInt_32 capacity = 50 + rnd2 * 50;
//		printf("alpha 0: %.3f, %.3f\n", probability, rnd1);
		
		if (probability >= rnd1)
		{
		    add_Arrow(v, u, capacity);
		    add_Arrow(u, v, capacity);
		}
	    }
	}	
    }
    ...
    void s_DirectedGraph::generate_DirectedWaxman(sInt_32 N_vertices, sDouble alpha, sDouble beta, sInt_32 x_grid_max, sInt_32 y_grid_max)
    {
	WaxmanNodes_vector waxman_Nodes;
	WaxmanNodes_vector selected_Nodes;
...
/*----------------------------------------------------------------------------*/
    ...
    void s_DirectedGraph::add_Vertex(void)
    {
	m_Vertices.push_back(s_Vertex(m_Vertices.size()));
    }


    void s_DirectedGraph::add_Vertex(sDouble capacity)
    {
	m_Vertices.push_back(s_Vertex(m_Vertices.size(), capacity));	
    }
    

    void s_DirectedGraph::add_Vertices(sInt_32 Vertex_cnt)
    {
	while (Vertex_cnt-- > 0)
	{
	    m_Vertices.push_back(s_Vertex(m_Vertices.size()));
    ...
    sInt_32 s_DirectedGraph::get_VertexCount(void) const
    {
	return m_Vertices.size();
    }
    ...
    const s_Vertex* s_DirectedGraph::get_Vertex(sInt_32 id) const
    {
	return &m_Vertices[id];
    }


    sInt_32 s_DirectedGraph::get_ArcCount(void) const
    {
	return m_Arc_cnt;
    }


    void s_DirectedGraph::add_Arrow(sInt_32 u_id, sInt_32 v_id, sDouble capacity)
    {
	sASSERT(m_Vertices.size() > u_id && m_Vertices.size() > v_id);
	sASSERT(u_id != v_id);

	m_Arcs.push_back(s_Arc(m_Arc_cnt++, &m_Vertices[u_id], &m_Vertices[v_id], capacity));
	s_Arc &inserted_arc = m_Arcs.back();

	m_Vertices[v_id].m_in_Neighbors.push_back(&inserted_arc);
	m_Vertices[u_id].m_out_Neighbors.push_back(&inserted_arc);
    }


    void s_DirectedGraph::add_Edge(sInt_32 u_id, sInt_32 v_id, sDouble capacity)
    {
	sASSERT(m_Vertices.size() > u_id && m_Vertices.size() > v_id);
	sASSERT(u_id != v_id);

	m_Arcs.push_back(s_Arc(m_Arc_cnt++, &m_Vertices[u_id], &m_Vertices[v_id], capacity));
	s_Arc &inserted_arc_uv = m_Arcs.back();

	m_Vertices[v_id].m_in_Neighbors.push_back(&inserted_arc_uv);
	m_Vertices[u_id].m_out_Neighbors.push_back(&inserted_arc_uv);

	m_Arcs.push_back(s_Arc(m_Arc_cnt++, &m_Vertices[v_id], &m_Vertices[u_id], capacity));
	s_Arc &inserted_arc_vu = m_Arcs.back();

	m_Vertices[u_id].m_in_Neighbors.push_back(&inserted_arc_vu);
	m_Vertices[v_id].m_out_Neighbors.push_back(&inserted_arc_vu);	
    }


    bool s_DirectedGraph::is_Adjacent(sInt_32 u_id, sInt_32 v_id) const
    {
	sASSERT(m_Vertices.size() > u_id && m_Vertices.size() > v_id);

	if (is_LinkedTo(u_id, v_id) || is_LinkedTo(v_id, u_id))
	{
	    return true;
	}
	return false;
    ...
    bool s_DirectedGraph::is_LinkedTo(sInt_32 u_id, sInt_32 v_id) const
    {
	if (m_Vertices[u_id].m_out_Neighbors.size() < m_Vertices[v_id].m_in_Neighbors.size())
	{
	    for (s_Vertex::Neighbors_vector::const_iterator u_neighbor = m_Vertices[u_id].m_out_Neighbors.begin(); u_neighbor != m_Vertices[u_id].m_out_Neighbors.end(); ++u_neighbor)
	    {
		if ((*u_neighbor)->m_target->m_id == v_id)
		{
		    return true;
...
/*----------------------------------------------------------------------------*/
    ...
    void s_DirectedGraph::to_Screen(const sString &indent) const
    {
	to_Stream(stdout, indent);
    }


    void s_DirectedGraph::to_Screen_vertices(const sString &indent) const
    {
	to_Stream_vertices(stdout, indent);
    }


    void s_DirectedGraph::to_Stream(FILE *fw, const sString &indent) const
    {
	fprintf(fw, "%s_DirectedGraph: (|V|=%ld |E|=%ld) [\n", indent.c_str(), m_Vertices.size(), m_Arcs.size());
	...
	fprintf(fw, "%s%sE = {\n", indent.c_str(), s_INDENT.c_str());
	{	   
	    for (Arcs_list::const_iterator arc = m_Arcs.begin(); arc != m_Arcs.end(); ++arc)
	    {
		arc->to_Stream(fw, indent + s2_INDENT);
	    }
	    fprintf(fw, "%s%s}\n", indent.c_str(), s_INDENT.c_str());
	}
    ...
    void s_DirectedGraph::to_Stream_vertices(FILE *fw, const sString &indent) const
    {
	fprintf(fw, "%s_DirectedGraph: (|V|=%ld, |E|=%ld) [\n", indent.c_str(), m_Vertices.size(), m_Arcs.size());
