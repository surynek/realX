...
// s_Vertex
    ...
    public:
	typedef std::list<s_Arc*> Neighbors_list;
	typedef std::vector<s_Arc*> Neighbors_vector;
	typedef std::vector<sInt_32> VertexIDs_vector;

    public:
	s_Vertex();
	s_Vertex(sInt_32 id);
	s_Vertex(sInt_32 id, sDouble capacity);
	s_Vertex(const s_Vertex &vertex);
	const s_Vertex& operator=(const s_Vertex &vertex);

	sInt_32 calc_InNeighborCount(void) const;
	sInt_32 calc_OutNeighborCount(void) const;	
...
// s_Arc
    ...
    public:
	s_Arc();
	s_Arc(sInt_32 id, s_Vertex *source, s_Vertex *target);
	s_Arc(sInt_32 id, s_Vertex *source, s_Vertex *target, sDouble capacity);
	s_Arc(const s_Arc &arc);
	const s_Arc& operator=(const s_Arc &arc);
...
// s_DirectedGraph
	...
	typedef std::vector<s_Vertex> Vertices_vector;
	typedef std::list<s_Vertex*> Vertices_list;

	typedef std::vector<s_Arc> Arcs_vector;
	typedef std::list<s_Arc> Arcs_list;	
	/*----------------------------------------------------------------------------*/

	
	struct WaxmanNode
	{
	    WaxmanNode(sInt_32 x, sInt_32 y)
	    : m_x(x), m_y(y)
	    {
		// nothing
	    }
	...
	typedef std::vector<WaxmanNode> WaxmanNodes_vector;	
	/*----------------------------------------------------------------------------*/	

    public:
	s_DirectedGraph();
	s_DirectedGraph(sInt_32 N_vertices);
		
	s_DirectedGraph(const s_DirectedGraph &directed_graph);
	const s_DirectedGraph& operator=(const s_DirectedGraph &directed_graph);
	~s_DirectedGraph();
	/*----------------------------------------------------------------------------*/

	void generate_UndirectedWaxman(sInt_32 N_vertices, sDouble alpha, sDouble beta, sInt_32 x_grid_max, sInt_32 y_grid_max);
	void generate_DirectedWaxman(sInt_32 N_vertices, sDouble alpha, sDouble beta, sInt_32 x_grid_max, sInt_32 y_grid_max);	
	/*----------------------------------------------------------------------------*/	

	void add_Vertex(void);
	void add_Vertex(sDouble capacity);
	void add_Vertices(sInt_32 Vertex_cnt = 1);
	...
	sInt_32 get_ArcCount(void) const;	
	void add_Arrow(sInt_32 u_id, sInt_32 v_id, sDouble capacity);
	void add_Edge(sInt_32 u_id, sInt_32 v_id, sDouble capacity);
